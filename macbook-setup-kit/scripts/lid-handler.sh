...
while true; do
    ...
    if [ "$st" = "closed" ] && [ "$closed_at" -gt 0 ] && [ $((now - closed_at)) -ge "$TIMEOUT" ]; then
        # antes de apagar: reactivar WiFi/BT para que el estado guardado sea "ON"
        # (si no, NetworkManager/systemd-rfkill guardan "apagado" y al encender vuelven apagados)
        nmcli radio wifi on 2>/dev/null
        rfkill unblock bluetooth 2>/dev/null
        sleep 2                     # dejar que el estado "on" se persista antes del apagado
        systemctl poweroff
    fi
